...
const LocalFuncInventoryArray = () => {
  ...
  const LocalInventoryItem1 = LocalFuncPrepareInventory({
  ...
  const LocalInventoryItem2 = LocalFuncPrepareInventory({
    inItemName: "Dyes & Chemicals-Kgs-3204-18%",
    inTaxPer: 18, inUom: "Kgs",
    inRate: 100,
    inQty: 10
  });
...
const LocalFuncPrepareInventory = ({ inItemName, inTaxPer, inUom, inRate, inQty }) => {
  ...
  const LocalRateWithTax = inRate * (1 + (inTaxPer / 100));
  const LocalAmount = inRate * inQty;
